...
def mouse_click(event, x, y,
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if case == 1:
            ...
            filename = '../../data/training_patches/bio/'+str(count)+'_bio.png'
            cv2.imwrite(filename, dst)

        if case == 2:
            print("No Biotop at: ",x,y,' #Patch: ',count)
            filename = '../../data/training_patches/no_bio/'+str(count)+'_no_bio.png'
            cv2.imwrite(filename, dst)


def crop_patch(files, bio_folder):
    cropfile = [x for x in files if "crop"  in x][0]
    rawfile = [x for x in files if "crop" not in x][0]
...
if __name__ == "__main__":
    ...
    for folder in folders:
        ...
        files[:] = [x for x in files if ".DS_Store" not in x and "B" not in x and "M" not in x]
